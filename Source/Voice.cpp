...
shared_ptr<Processor> Voice::getProcessor(String name) { return processorMap[name]; }
shared_ptr<Processor> Voice::getModulator(int index) { return modulators[index]; }
shared_ptr<Processor> Voice::getProcessor(Index index) { return graphManager.getModuleProcessor(index); }
shared_ptr<Processor> Voice::getModulator(String name) { return modulatorMap[name]; }
...
void Voice::addModulator(shared_ptr<Module> module) {
...
shared_ptr<Processor> Voice::addBlock(shared_ptr<Block> block) {
...
void Voice::addChild(shared_ptr<Block> parent, Index index) {
...
void Voice::connect(shared_ptr<Modulation> connection) {
...
void Voice::disconnect(shared_ptr<Modulation> connection) {
  auto targetProcessor = processorMap[connection->target->name];

  if (connection->isOscGainEnvelope()) {
    auto osc = static_pointer_cast<OscillatorProcessor>(targetProcessor);
...
shared_ptr<ModulationInput> Voice::getModulationInput(shared_ptr<Modulation> modulationConnection) {
...
shared_ptr<ModulationInput> Voice::getModulationInput(String source, String target, int parameterIndex) {
