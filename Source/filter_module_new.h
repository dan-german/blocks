...
public:
  FilterModule(int number): Block("filter", number) {
    add({ .name = "model", .value_scale = ValueScale::kIndexed, .max = 7.0, .string_lookup = strings::kFilterModelNames });
    add({ .name = "style", .value_scale = ValueScale::kIndexed, .max = 4.0, .string_lookup = strings::kFilterStyleNames, });
