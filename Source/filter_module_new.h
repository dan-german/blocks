...
public:
  FilterModule(int number): Block("filter", number) {
    add({ .name = "model", .max = 8.0, .string_lookup = strings::kFilterModelNames, .value_scale = ValueScale::kIndexed });
    add({ .name = "style", .max = 3.0, .string_lookup = strings::kFilterStyleNames, .value_scale = ValueScale::kIndexed });
