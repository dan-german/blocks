...
json Sample::stateToJson() {
  ...
  juce::String encoded = juce::Base64::toBase64(pcm_data.get(), sizeof(int16_t) * data_->length);
  data["samples"] = encoded.toStdString();
  if (data_->stereo) {
    utils::floatToPcmData(pcm_data.get(), data_->right_buffers[kUpsampleTimes].get(), data_->length);
    juce::String encoded_stereo = juce::Base64::toBase64(pcm_data.get(), sizeof(int16_t) * data_->length);
...
void Sample::jsonToState(json data) {
  ...
  juce::MemoryOutputStream decoded(length * sizeof(int16_t));
  std::string wave_data = data["samples"];
  juce::Base64::convertFromBase64(decoded, wave_data);
  ...
  if (data.count("samples_stereo")) {
    juce::MemoryOutputStream decoded_stereo(length * sizeof(int16_t));
    std::string wave_data_stereo = data["samples_stereo"];
    juce::Base64::convertFromBase64(decoded_stereo, wave_data_stereo);
